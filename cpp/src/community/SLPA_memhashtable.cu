...
struct get_label
{
  ...
  {
    return thrust::make_tuple<int32_t,int>(thrust::get<0>(max_per_row[col[thread_id]]), 1);
...
struct get_frequency
{
    ...
    int n;
    //int * _col, int * _mem, int * _memnnz,
    get_frequency(int _n, thrust::universal_vector<int> &_row, thrust::universal_vector<thrust::tuple<int32_t,int> > &_labellist, thrust::universal_vector<int> &_row_id) {
        row= thrust::raw_pointer_cast(_row.data());
        labellist = thrust::raw_pointer_cast(_labellist.data());
        row_id = thrust::raw_pointer_cast(_row_id.data());
        n = _n;
  ...
  thrust::tuple<int32_t,int> operator()(unsigned int thread_id)
  {
    ...
    thrust::tuple<int32_t,int> val(thrust::get<0>(labellist[thread_id]), count);
    return val;
  }
};
...
//     int * row;
//     thrust::tuple<int32_t,int> *labellist;
//     //cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int, cuda::std::__4::__detail::thread_scope_device, cuco::cuda_allocator<char>>::device_mutable_view<18446744073709551615UL> map1;
//     //cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int, cuda::std::__4::__detail::thread_scope_device, cuco::cuda_allocator<char>>::device_view<18446744073709551615UL> map2;
//     //int * _col, int * _mem, int * _memnnz,
//     cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int> map;
//     update_memory(thrust::universal_vector<int> &_row, thrust::universal_vector<thrust::tuple<int32_t,int> > &_max_per_row, thrust::universal_vector<thrust::tuple<int32_t,int> > &_labellist, 
//     cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int> &_map) {
//         row= thrust::raw_pointer_cast(_row.data());
//         labellist = thrust::raw_pointer_cast(_labellist.data());
//         max_per_row = thrust::raw_pointer_cast(_max_per_row.data());
//         map = _map;
//         // auto map1 = _map.get_device_mutable_view();
//         // auto map2 = _map.get_device_view();
//     }
//   __device__
//   void operator()(unsigned int i)
//   {
//     auto map1 = _map.get_device_mutable_view();
//     map1.insert(cuco::make_pair<custom_key_type, int>(custom_key_type{(int32_t)i,(int32_t)thrust::get<0>(labellist[row[i]])}, (int)thrust::get<1>(labellist[row[i]])));
//     auto map2 = _map.get_device_view();
//     auto res = map2.find(custom_key_type{i,thrust::get<0>(labellist[row[i]])});
//     if(res->second.load() > thrust::get<1>(max_per_row[i])) {
//       max_per_row[i] = thrust::make_tuple(res->first.load().b, res->second.load());
//       //mpr[i] = thrust::make_tuple(-1, -1);
//     }
//   }
// };
...
//     int n;
//     int T;
//     thrust::tuple<uint32_t, int> * max_per_row;
//     //int * _col, int * _mem, int * _memnnz,
//     update_memory(int _n, thrust::universal_vector<int> &_row, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_mem, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_labellist, thrust::universal_vector<int> &_memnnz, int _T,
//     thrust::universal_vector<thrust::tuple<uint32_t,int>> &_max_per_row) {
//         row= thrust::raw_pointer_cast(_row.data());
//         mem = thrust::raw_pointer_cast(_mem.data());
//         labellist = thrust::raw_pointer_cast(_labellist.data());
//         memnnz = thrust::raw_pointer_cast(_memnnz.data());
...
struct post_process
{
    ...
    float r;
    //int * _col, int * _mem, int * _memnnz,
    post_process(int _n, cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int> &_mem, int _T, float _r) {
        slots = _mem.get_device_mutable_view().get_slots();
        n = _n;
        T = _T;
        r = _r;
    }
  __device__
  thrust::tuple<custom_key_type, int> operator()(unsigned int thread_id)
  {
    thrust::tuple<custom_key_type, int> res(custom_key_type{-1,-1},0);
      // if(slots[thread_id].first.load().a != -1)
      // printf("thread %d has values %d against %f\n", thread_id, slots[thread_id].second.load(),  (float)(T*r));
    if(slots[thread_id].second.load() <= 0 || slots[thread_id].second.load() < (float)(T*r)) {
      return res;
    }
    //return thrust::make_tuple<custom_key_type, int>(custom_key_type{slots[thread_id].first.load().a, slots[thread_id].first.load().b}, slots[thread_id].second.load());
    return thrust::make_tuple<custom_key_type, int>(slots[thread_id].first.load(), slots[thread_id].second.load());
...
int main(int argc, char *argv[]) {
  ...
  static constexpr int capacity = 2048;
   using map_type =
    cuco::static_reduction_map<cuco::reduce_add<Value>, custom_key_type, Value, cuda::thread_scope_block>;
    ...
    int ne = T;
    cuco::static_reduction_map<cuco::reduce_add<Value>, custom_key_type, Value> mem(n*ne*2, empty_key_sentinel);
    ...
    //   max_per_row[i] = thrust::reduce(labellist.begin()+row[i], labellist.begin() + row[i+1], labellist[row[i]], tupleMax());
    // }
    // for(int i = 0; i < n; i++) {
    //   std::cout << thrust::get<0>(max_per_row[i]) << " " << thrust::get<1>(max_per_row[i]) << std::endl;
    // }
    // for(int i = 0; i < n; i++) {
    //   int offset = row[i+1] - row[i];
    //   max_per_row[i] = thrust::reduce(mem.begin()+i*ne, mem.begin() + i*ne + ne, mem[i*ne], tupleMax());
    // }
    // for(int i = 0; i < n; i++) {
    //   std::cout << thrust::get<0>(max_per_row[i]) << " " << thrust::get<1>(max_per_row[i]) << std::endl;
    // }
    ...
    for(int k = 1; k < T; k++) {
        ...
        //             });
        get_label s(col, max_per_row, row_id);
        thrust::transform(thrust::counting_iterator(0), thrust::counting_iterator(cols), labellist.begin(), s);
        ...
        get_frequency l1(n, row, labellist, row_id);
        ...
        // }
        // update_memory l3(row, max_per_row, labellist, mem);
        // thrust::for_each(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(n), l3);
        
        thrust::for_each(thrust::counting_iterator(0),
                     thrust::counting_iterator(n),
                    [m = mem.get_device_mutable_view(), m2 = mem.get_device_view(), ll = thrust::raw_pointer_cast(labellist.data()), r = thrust::raw_pointer_cast(row.data()), mpr = thrust::raw_pointer_cast(max_per_row.data())]
                     __device__ (auto i) mutable {
                      //printf("%d, %d\n", thrust::get<0>(mpr[i]), thrust::get<1>(mpr[i]));
                      
                      m.insert(cuco::make_pair<custom_key_type, int>(custom_key_type{(int32_t)i,(int32_t)thrust::get<0>(ll[r[i]])}, 1));
                      //m.insert(cuco::make_pair<custom_key_type, int>(custom_key_type{rowid[i], thrust::get<0>(mpr[c[i]])}, 1));
                      auto res = m2.find(custom_key_type{i,thrust::get<0>(ll[r[i]])});
                      if(res->second.load() > thrust::get<1>(mpr[i])) {
                        mpr[i] = thrust::make_tuple(res->first.load().b, res->second.load());
                        //mpr[i] = thrust::make_tuple(-1, -1);
                        //printf("%d, %d\n", thrust::get<0>(mpr[i]), thrust::get<1>(mpr[i]));
                      }
                    });
    ...
    thrust::universal_vector<thrust::tuple<custom_key_type, int>> result(n*ne*2); 
    post_process p(n, mem, T, r);
    thrust::transform(thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(n*ne*2), result.begin(), p);
    cudaDeviceSynchronize();
    ...
    //   int count = 0;
    //   std::cout << "Vertex " << i << " is in communities ";
    //   for(int j = 0; j < n*ne*2; j++) {
    //     if(thrust::get<0>(result[j]).a == i && thrust::get<1>(result[j]) > 0) {
    //       std::cout << thrust::get<0>(result[j]).b << " ";
    //       count++;
    //     }
    //   }
    //   if(count > 1)
