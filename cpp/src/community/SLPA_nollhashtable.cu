...
struct write_back
{
    thrust::tuple<int32_t, int> * new_max;
    ...
    //int * _col, int * _mem, int * _memnnz,
    write_back(thrust::universal_vector<thrust::tuple<int32_t, int>> &_new_max, cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int> &_map) {
        new_max = thrust::raw_pointer_cast(_new_max.data());
  ...
  void operator()(unsigned int thread_id)
  {
    if(slots[thread_id].second >= 1) {
      ...
      auto val = slots[thread_id].second.load();
      if(val > thrust::get<1>(new_max[range])) {
        atomicCAS(&(thrust::get<0>(new_max[range])), thrust::get<0>(new_max[range]), key);
        atomicCAS(&(thrust::get<1>(new_max[range])), thrust::get<1>(new_max[range]), val);
      }
...
struct max_tie
{
    ...
    cuco::pair<cuda::__4::atomic<custom_key_type, cuda::std::__4::__detail::thread_scope_device>, cuda::__4::atomic<int, cuda::std::__4::__detail::thread_scope_device>> * slots;
    unsigned int seed;
    max_tie(thrust::universal_vector<int> &_row, thrust::universal_vector<thrust::tuple<int32_t, int>> &_new_max, thrust::universal_vector<thrust::tuple<int32_t, int>> &_labellist,cuco::static_reduction_map<cuco::reduce_add<int>, custom_key_type, int> &_map,
     unsigned int _seed) {
        row = thrust::raw_pointer_cast(_row.data());
        labellist = thrust::raw_pointer_cast(_labellist.data());
        new_max = thrust::raw_pointer_cast(_new_max.data());
  ...
  void operator()(unsigned int thread_id)
  {
    ...
    thrust::uniform_real_distribution<float> u01(0,1);
     if(slots[thread_id].second >= 1) {
      ...
      auto val = slots[thread_id].second.load();
      if(val == thrust::get<1>(new_max[range])) {
        if((int)truncf(u01(rng)) == 1) {
          //labellist[row[range]] = thrust::make_tuple<int32_t, int>(key, val);
          atomicCAS(&(thrust::get<0>(labellist[row[range]])), thrust::get<0>(labellist[row[range]]), key);
          atomicCAS(&(thrust::get<1>(labellist[row[range]])), thrust::get<1>(labellist[row[range]]), val);

        } 
        //else {
        //   labellist[row[range]] = new_max[range];
        // }
      }
...
int main(int argc, char *argv[]) {
  ...
  static constexpr int capacity = 2048;
   using map_type =
    ...
    thrust::universal_vector<thrust::tuple<int32_t,int> > labellist(cols);
    ...
    for(int k = 1; k < T; k++) {
        ...
        thrust::universal_vector<thrust::tuple<int32_t, int>> new_max(n);
        thrust::fill(new_max.begin(), new_max.end(), -1);
        write_back wb(new_max, map);
        thrust::for_each(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(cols*2), wb);
        cudaDeviceSynchronize();
        for(int i = 0; i < n; i++) {
          std::cout << thrust::get<0>(new_max[i]) << " " << thrust::get<1>(new_max[i]) << std::endl;
        }
        std::cout << std::endl;
        ...
        thrust::for_each(thrust::counting_iterator(0),
                    ...
                    [nm = thrust::raw_pointer_cast(new_max.data()), r = thrust::raw_pointer_cast(row.data()), ll = thrust::raw_pointer_cast(labellist.data())]
                     __device__ (auto i) mutable {
                       ll[r[i]] = nm[i];
                    });
        // max_tie mt(row, new_max, labellist, map, seed);
        //  thrust::for_each(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(cols*2), mt);
