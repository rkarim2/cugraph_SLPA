...
struct get_label
{
    ...
    //int * _col, int * _mem, int * _memnnz,
    get_label(thrust::universal_vector<int> &_col, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_max_per_row) {
        col= thrust::raw_pointer_cast(_col.data());
  ...
  thrust::tuple<int,int> operator()(unsigned int thread_id)
  {
    thrust::tuple<uint32_t,int> val(thrust::get<0>(max_per_row[col[thread_id]]), 1);
    return val;
  }
};
...
struct sort_per_row
{
    ...
    int n;
    //int * _col, int * _mem, int * _memnnz,
    sort_per_row(int _n,thrust::universal_vector<int> &_row, thrust::universal_vector<int> &_col, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_labellist, thrust::universal_vector<int> &_row_id) {
        row= thrust::raw_pointer_cast(_row.data());
        col= thrust::raw_pointer_cast(_col.data());
        labellist = thrust::raw_pointer_cast(_labellist.data());
        row_id = thrust::raw_pointer_cast(_row_id.data());
...
struct check_ties
{
    ...
    int n;
    //int * _col, int * _mem, int * _memnnz,
    check_ties(int _n,thrust::universal_vector<int> &_row, thrust::universal_vector<int> &_col, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_labellist, thrust::universal_vector<int> &_row_id,
...
struct break_ties
{
    ...
    int n;
    unsigned int seed;
    break_ties(int _n,thrust::universal_vector<int> &_row, thrust::universal_vector<int> &_col, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_labellist, thrust::universal_vector<int> &_row_id,
...
struct post_process
{
    ...
    float r;
    //int * _col, int * _mem, int * _memnnz,
    post_process(int _n, thrust::universal_vector<thrust::tuple<uint32_t,int> > &_mem, int _T, float _r) {
        mem = thrust::raw_pointer_cast(_mem.data());
        n = _n;
        T = _T;
        r = _r;
...
int main(int argc, char *argv[]) {
  ...
  static constexpr int capacity = 2048;
   using map_type =
    ...
    int ne = T;
    thrust::universal_vector<thrust::tuple<uint32_t,int> > mem(n*ne);
    //std::cout << mem.size() << std::endl;
    thrust::universal_vector<int> memnnz(n);
    for(int i = 0; i < n; i++) {
            thrust::tuple<uint32_t,int> init(i, 1);
        for(int j = 0; j < ne; j++) {
                mem[i*ne+j] = init;
    ...
    thrust::universal_vector<thrust::tuple<uint32_t,int> > labellist(cols);
    ...
    double timepp = 0;
    unsigned int seed;
    thrust::universal_vector<thrust::tuple<uint32_t,int>> max_per_row(n);
    for(int i = 0; i < n; i++) {
    ...
    for(int k = 1; k < T; k++) {
        ...
        // for(int i = 0; i < n; i++) {
        //     int offset = row[i+1] - row[i];
        //     max_per_row[i] = thrust::reduce(thrust::device, mem.begin()+i*ne, mem.begin() + i*ne + ne, mem[i*ne], tupleMax());
        // }
        ...
        thrust::universal_vector<int> labellistnnz1(n);
        thrust::fill(labellistnnz1.begin(), labellistnnz1.end(), 0);
        int numblocks = ceil((float)cols/(float)N);
        //std::cout << "this is numblocks " << numblocks << std::endl;
        // int N = 256;
        // if(cols < 256) {
        //   N = cols;
        //   static constexpr int capacity2 = 56;
        //   send_aggregate_label<map_type, capacity2><<<numblocks,N>>>(n, thrust::raw_pointer_cast(row.data()), thrust::raw_pointer_cast(col.data()), thrust::raw_pointer_cast(row_id.data()), thrust::raw_pointer_cast(max_per_row.data()), thrust::raw_pointer_cast(labellistnnz1.data()), thrust::raw_pointer_cast(labellist.data()),
        //   thrust::raw_pointer_cast(locks.data()));
        // }
        // else {
        send_aggregate_label<map_type, capacity><<<numblocks,N>>>(n, thrust::raw_pointer_cast(row.data()), thrust::raw_pointer_cast(col.data()), thrust::raw_pointer_cast(row_id.data()), thrust::raw_pointer_cast(max_per_row.data()), thrust::raw_pointer_cast(labellistnnz1.data()), thrust::raw_pointer_cast(labellist.data()));
        // }
